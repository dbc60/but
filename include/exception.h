...
/**
 * @brief but_expected_failure is meant to be thrown from test cases, but not caught within the test. The
 * test driver should catch it and NOT report it as a failed test case.
 */
extern BUTExceptionReason but_expected_failure;

/**
 * @brief but_test_exception is useful during testing. It's meant to be thrown and caught within test cases. If it is not caught,
 * then the test driver will report it as an uncaught exception. It's mostly used within the test suite for the exception
 * library.
 */
extern BUTExceptionReason but_test_exception;

/**
 * @brief but_not_implemented is useful during development to track unimplemented features.
 */
extern BUTExceptionReason but_not_implemented;

/**
 * @brief but_invalid_value can be thrown by a function that has been passed an argument with an invalid value.
 */
extern BUTExceptionReason but_invalid_value;

/**
 * @brief but_internal_error can be thrown by a function that is in a bad state. This probably means there's a bug in its
 * component/library.
 */
extern BUTExceptionReason but_internal_error;

/**
 * @brief but_invalid_address can be thrown by a function that has been passed an invalid address, such as one that has already
 * been freed or one that is not within the bounds of a valid memory region.
 */
 ...
 *
 * There is a limit of BUT_MAX_DETAILS_LENGTH bytes in the buffer used for formatting the reason.
 *
 ...
 * BUT_CATCH_ALL block.
 */
#define BUT_RETHROW but_throw(but_env_.reason, but_env_.details, but_env_.file, but_env_.line)

#define BUT_REASON  (but_env_.reason)
#define BUT_DETAILS (but_env_.details)
#define BUT_PATH    (but_env_.file)
...
#define BUT_TRY                                                         \
    do {                                                                \
        ...
        BUTExceptionContext *but_ctx_    = but_get_exception_context(); \
 ...
 */
#define BUT_THROW_FN(name) void name(BUTExceptionReason reason, char const *details, char const *file, u32 line)
